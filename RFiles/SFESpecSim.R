...
# Quantlet:     SFESpecSim
# ------------------------------------------------------------------------------
# Description:  SFESpecSim simulates AR(1), AR(2), MA(1), SAR(1) processes 
...
library(TSA) # library TSA is needed to compute theoretical spectral density
...
# AR(2) processes
phi1 = 0.9 ; phi2 = -0.75 # AR(2) parameters
...
phi1 = -0.1 ; phi2 = 0.5 # AR(2) parameters
x    = arima.sim(n = n,model = list(ar=c(phi1, phi2)), rand.gen=function(n, ...) rnorm(n,0,sigma)) # simulate process
l    = ARMAspec(model = list(ar = c(phi1, phi2)), plot=F) # compute theoretical spectral density
...
phi1   = 0.5;   PHI1 = 0.5    # Reset theta for Seasonal AR
l      = ARMAspec(model=list(ar = phi1, seasonal = list(sar = PHI1, period=12)), col="magenta3", lwd=3, cex.axis=0.8, cex.lab=0.9, plot=F)
